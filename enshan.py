...
def get_points(cookie):
    ...
    initialize.info_message(f"{user_name}---{points}---{user_group}")
    ...
    for number, cookie in enumerate(cookies.split("&")):
        ...
        if not ('TWcq_2132_saltkey' in cookie or 'TWcq_2132_auth' in cookie):
            initialize.error_message(
                f"第{number + 1}个账号的cookie中未包含TWcq_2132_saltkey或TWcq_2132_auth字段，请检查cookie")
            break
        ...
        if status_code == 200:
            ...
            for tr in trs:
                ...
                if tds[0].text == "每天登录" and tds[5].text[:10] == datetime.datetime.now().strftime("%Y-%m-%d"):
                    initialize.info_message(f"第{number + 1}个账号模拟登录成功")
                    get_points(cookie)  # 获取用户信息
                    break
            else:
                initialize.error_message(f"第{number + 1}个账号模拟登录失败")
        else:
            initialize.error_message(f"第{number + 1}个账号可能cookie过期")
